...
#include <excpt.h>
#include <string>
void AddNewLineToDotaChatLog( std::string s );
#include "VEHtoSEH.h"
BOOL IsVEHex = FALSE;
...
{
	IsVEHex = TRUE;
	AddNewLineToDotaChatLog( "Test: Found VEH Exception!" );
	...
	while ( ( LONG )Registration != -1 ) // -1 means end of chain
	{
		...
		case ExceptionContinueExecution:
			...
			{
				AddNewLineToDotaChatLog( "Test: Exception can be skipped :O ! Code:" 
					+ std::to_string( ex->ExceptionCode ));
...
{
	RemoveVectoredExceptionHandler( ( PVECTORED_EXCEPTION_HANDLER )&VectoredSEH );
}*/