...
typedef void *( __fastcall * GAME_SendPacket_p ) ( CDataStore* packet, DWORD );
...
void __stdcall Packet_Send( int unused )
{
	...
	//PacketSendPacket( &BytesToSend[ 0 ], BytesToSend.size( ) );
...
void InitializePacketHandler( int GameVersion )
{
	...
	else if ( GameVersion == 0x27a )
	{
		...
		CNetCommandTeamChangeAlliance_org = ( RecvNetCommand )( GameDll + 0x2FB3C0 );
		MH_CreateHook( CNetCommandTeamChangeAlliance_org, &CNetCommandTeamChangeAlliance_my, reinterpret_cast< void** >( &CNetCommandTeamChangeAlliance_ptr ) );
		MH_EnableHook( CNetCommandTeamChangeAlliance_org );
		CDataStoreByteReader = ( sub_6F4C2C10 )( GameDll + 0x04D3B0 );
	}
}
