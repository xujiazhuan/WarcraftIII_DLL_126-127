 ...
 */
 ...
 07.10.2017 remove all f**ing includes from this header.
 by Karaulov
					...
					if ( bSendRequestSucceed )
					{
						...
						if ( ::WinHttpReceiveResponse( hRequest, NULL ) )
						{
							...
							if ( bResult || ( !bResult && ( ::GetLastError( ) == ERROR_INSUFFICIENT_BUFFER ) ) )
							{
								...
								if ( szHeader != NULL )
								{
										...
										WINHTTP_NO_HEADER_INDEX ) )
									{
										...
										std::wsmatch result;
										wregex regExp( L"charset=([A-Za-z0-9\\-_]+)");
								
										if ( !m_bForceCharset )
										{
											regExp = wregex( L"charset=([A-Za-z0-9\\-_]+)" );
											if ( std::regex_match( m_responseHeader, result, regExp ) && result.size( ) > 0 )
											{
												m_responseCharset = result[ 1 ];
											}
										}

										regExp = wregex( L"Content-Length:\\s+([0-9]+)");
										if ( std::regex_match( m_responseHeader, result, regExp ) && result.size( ) > 0 )
										{
											m_responseByteCount = ( unsigned int )_wtoi( result[ 1 ].str( ).c_str( ) );
										}

										regExp = wregex( L"Location:\\s+([0-9]+)");
										if ( std::regex_match( m_responseHeader, result, regExp ) && result.size( ) > 0 )
										{
											m_location = result[ 1 ];
										}
										
										regExp = wregex( L"Set-Cookie:\\s*(.+?)\\n");
										if ( std::regex_match( m_responseHeader, result, regExp ) && result.size( ) > 0 )
										{
											wstring tmpstr = m_responseHeader;
											while ( std::regex_search( tmpstr, result, regExp ) )
											{
												m_responseCookies += result[ 1 ];
							...
							if ( verb == L"GET" && !disableAutoRedirect )
							{
								...
								wregex regExp;
								regExp = L"(<html>)";
								if ( std::regex_match( m_responseContent, result, regExp ) && result.size( ) > 0 )
								{
									if ( !std::regex_match( m_responseContent, result, regExp ) || result.size( ) <= 0 )
									{
