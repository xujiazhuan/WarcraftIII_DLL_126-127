...
void* CQuantizer::CreateNode(unsigned int nLevel, unsigned int nColorBits, unsigned int* pLeafCount, Node** pReducibleNodes)
{
...
unsigned char CQuantizer::GetNearestIndex(BGRAPix* c, BGRAPix* pal)
{
	...
	for(unsigned long i=0, k=0, distance=2147483647; i<m_nLeafCount; i++) {
		k = ((pal[i].B-c->B)*(pal[i].B-c->B) + (pal[i].G-c->G)*(pal[i].G-c->G) + (pal[i].R-c->R)*(pal[i].R-c->R));
		if (k==0) {
